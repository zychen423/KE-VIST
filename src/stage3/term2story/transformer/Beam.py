...
class Beam():
    ...
    def __init__(self, size, device=False):
        ...
        self.next_pos = [torch.full((size,), Constants.PAD, dtype=torch.long, device=device)]
        self.next_pos[0][0] = 1

        # The sen pos at each time-step
        self.next_sen_pos = [torch.full((size,), Constants.PAD, dtype=torch.long, device=device)]
        self.next_sen_pos[0][0] = 1
    ...
    def advance(self, word_prob):
        ...
        for i, word in enumerate(self.next_ys[-1]):
            if word.item() in self.BOSs_set:
                tmp_pos[i] = 1
                tmp_sen_pos[i] = self.next_sen_pos[-1][self.prev_ks[-1][i]]+1
            else:
